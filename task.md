...
## Задание 25
Реализовал паттерн Visitor в классе Visitor в пакете ru.npcric.asparagus.trainerslog.service.visitor

## Признак реализации 
1. Класс Visitor является абстрактным посетителем содержащий методы visit для каждого отдельного типа
2. Конкретный посетитель - PrintVisitor, реализует интерфейс Visitor и реализует методы visit()
3. Базовым элементом является класс Element, конкретные элементы - Report и Invoice. Они содержат метод accept() для вызова методов посетителя 
