...
2. Статическое поле для хранения экземпляра: В классе определено статическое поле instance, которое хранит единственный экземпляр Logger
  
3. Статический метод для доступа к экземпляру: Метод getInstance() предоставляет глобальную точку доступа к единственному экземпляру класса. Он проверяет, был ли уже создан экземпляр, и создает его при первом вызове.
